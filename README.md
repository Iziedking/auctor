...
- `https://auctor.space/` — public authentication gateway.
- `https://auctor.space/chat` — command workspace and reviewable action previews.
- `https://auctor.space/audit` — evidence dossiers and execution history.
- `https://auctor.space/api/health` — effective capability health without credentials.
...
Open `http://127.0.0.1:3200/`. Keep `.env` outside version control. KeeperHub, database, AgentQA, notification, and model credentials are server-only.

## Configuration

Copy `.env.example` and provide the deployment-specific values. In production, provide a valid `KEEPERHUB_WALLET_ADDRESS`, KeeperHub credentials, PostgreSQL URL, authenticated AgentQA configuration, and `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID`. The public health endpoint reports only whether each capability is configured and reachable.

## Release and deployment

Pull requests run type checking, the complete unit suite, and the production build. A passing push to `main` builds an immutable image tagged with the commit SHA, publishes it to GHCR, runs database migrations on the VPS, replaces the application container, waits for health, and verifies the public authentication gateway.

The production environment keeps `.env`, database volumes, AgentQA data, and ingress configuration on the server. They are never committed or baked into an image. See [RUNBOOK.md](RUNBOOK.md) for operator setup and recovery procedures.
