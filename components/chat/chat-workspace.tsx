      ...
      <div className="chatBody">
        <section className="agentContext" aria-label="Agent context">
          ...
          <div>
            ...
            {agent?.khWalletAddress ? <div className="walletFunding"><b>{shortAddress(agent.khWalletAddress)}</b><button type="button" onClick={async()=>{try{await navigator.clipboard.writeText(agent.khWalletAddress!);setCopiedWallet(true);setTimeout(()=>setCopiedWallet(false),1600)}catch{}}}>{copiedWallet ? "Copied" : "Copy address"}</button></div> : <b>KeeperHub provisioning</b>}
            {agent?.khWalletAddress && <small className="walletFundingHint">Fund this wallet with Base Sepolia ETH before requesting a testnet swap.</small>}
          </div>
        </section>
...
function naturalReason(reason?:string){if(!reason)return "I couldn’t complete that safely yet. Tell me what asset, amount, chain, or condition you want.";const messages:Record<string,string>={token_not_found_on_chain:"I couldn’t find that token on this testnet. Use the token contract address, or confirm the symbol and network.",token_symbol_ambiguous_use_address:"That symbol matches multiple tokens on this network. Please send the token contract address.",token_approval_required_before_swap:"This token needs approval before it can be swapped. Approve it first, then retry the command.",uniswap_chain_unsupported:"That network is not enabled for swaps yet.",uniswap_provider_response_invalid:"Uniswap’s testnet quote service returned an invalid response. No transaction was submitted. Try the verified Base mainnet route instead."};if(messages[reason])return messages[reason];if(reason.includes("unsupported_intent"))return "I understood this as a conversation, but I need a little more detail to act. Ask about your portfolio, a token, research, or an exact guarded swap.";return humanize(reason)}
