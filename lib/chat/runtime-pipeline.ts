...
export function createRuntimeChatPipeline(config: RuntimeChatConfig, factories: RuntimeChatFactories = defaults) {
  ...
  return createLiveChatPipeline({ walletAddress: config.keeperhub.walletAddress as Address, simulator: factories.createLiveClient({ baseUrl: config.keeperhub.baseUrl, apiKey: config.keeperhub.apiKey }) });
}