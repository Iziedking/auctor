...
export async function handleApprovalRequest(input: unknown, deps: { readonly enabled: boolean; readonly pipeline: Pipeline; readonly execute: (preview: Extract<ChatPipelineResult, { kind: "preview" }>, recalledMemory: readonly string[]) => Promise<ApprovedResult> }) {
  if (!deps.enabled) return { status: 503 as const, body: { error: "mock_approval_unavailable" as const } };
  ...
  if (!isCapturedMockApprovalPreview(preview)) return { status: 409 as const, body: { error: "unsupported_mock_trade" as const, reason: "Only the captured 0.001 ETH to USDC trade on Base can be approved in mock mode." } };
