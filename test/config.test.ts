...
test("research providers parse a comma-separated category registry",()=>{const config=loadConfig({X402_RESEARCH_PROVIDERS:"coingecko|market|https://pro-api.coingecko.com/api/v3/x402/simple/price,allium|onchain|https://example.allium.so/x402/query"});assert.deepEqual(config.research.providers,[{name:"coingecko",category:"market",endpoint:"https://pro-api.coingecko.com/api/v3/x402/simple/price"},{name:"allium",category:"onchain",endpoint:"https://example.allium.so/x402/query"}])});
test("keyed sentiment and free news enable research without x402",()=>{const config=loadConfig({ADANOS_API_KEY:"sk_live_test",FREE_CRYPTO_NEWS_ENDPOINT:"https://cryptocurrency.cv/api/news"});assert.equal(config.capabilities.research,true);assert.equal(config.research.adanosApiKey,"sk_live_test");assert.equal(config.research.newsEndpoint,"https://cryptocurrency.cv/api/news")});
