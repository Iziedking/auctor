...
test("auth JSON parser explains injected wallet conflicts",async()=>{await assert.rejects(()=>readAuthJson(Response.json({error:"wallet_provider_conflict"},{status:401})),/another wallet extension/i)});
