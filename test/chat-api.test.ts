...
test("chat API routes natural language through the LLM after memory recall",async()=>{const calls:unknown[]=[];const session={async recallMemory(){return["Use Base only."]},async rememberDecision(){return null}};const conversation={async handle(input:any){calls.push(input);return{conversationId:"00000000-0000-4000-8000-000000000090",response:await pipeline.handle({text:input.text,correlationId:input.correlationId,recalledMemory:input.recalledMemory})}}};const language={async route(input:any){assert.deepEqual(input.memory,["Use Base only."]);return{text:"swap 1 USDC to WETH on base",reply:"I translated your request.",source:"llm" as const}}};const response=await handleChatRequest({text:"move one dollar into wrapped ether",correlationId:"api-llm-1"},{pipeline,session:session as never,identity:{user:"u",passphrase:"p",folder:"f"},conversation:conversation as never,agentId:"00000000-0000-4000-8000-000000000091",language,agent:{name:"Atlas",autonomyMode:"manual",dailyCapUsd:"5"}});assert.equal(response.status,200);assert.equal((calls[0] as any).text,"swap 1 USDC to WETH on base");assert.equal("interpretation" in response.body?response.body.interpretation:null,"I translated your request.")});
