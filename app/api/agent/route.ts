...
export async function GET(){const ctx=await context();if(!ctx)return NextResponse.json({error:"unauthorized"},{status:401});try{const rows=await ctx.database.db.select().from(agents).where(and(eq(agents.id,ctx.session.agentId),eq(agents.userId,ctx.session.userId))).limit(1);const agent=rows[0];if(agent&&isLegacyMainnetDefault(agent.allowedChains)){const upgraded=(await ctx.database.db.update(agents).set({allowedChains:[...KEEPERHUB_TEST_CHAIN_IDS]}).where(and(eq(agents.id,ctx.session.agentId),eq(agents.userId,ctx.session.userId))).returning())[0];return NextResponse.json(upgraded)}return NextResponse.json(agent)}finally{await ctx.database.close()}}
