...
import { temporalRecallQuery } from "../../../../lib/memory/events.ts";
...
export async function POST(request: Request) {
  ...
  if (!config.notifications.telegramToken || !config.database.url)
  ...
  const telegram = createTelegramNotifier({
    token: config.notifications.telegramToken,
    mockMode: false,
  });
  try {
    ...
    const rows = await database.db
      .select({
        ...
        agentId: channelConnections.agentId,
        name: agents.name,
        autonomyMode: agents.autonomyMode,
        walletAddress: agents.khWalletAddress,
        memoryKey: memoryIdentities.memoryKey,
      })
      .from(channelConnections)
      .innerJoin(agents, eq(agents.id, channelConnections.agentId))
      .innerJoin(
        memoryIdentities,
        eq(memoryIdentities.userId, channelConnections.userId),
      )
    ...
    if (!account) {
      await telegram.send({
        chatId,
        text: "Connect this chat first from Notifications on auctor.space, then send /connect CODE.",
      });
      return NextResponse.json({ ok: true });
    }
    if (!account.walletAddress) {
      await telegram.send({
        chatId,
        text: "Your KeeperHub agent wallet is not provisioned yet. Sign in at auctor.space and open Agent.",
      });
      return NextResponse.json({ ok: true });
    }
    const pipeline = createRuntimeChatPipeline({
      ...config,
      keeperhub: { ...config.keeperhub, walletAddress: account.walletAddress },
    });
    const identity = deriveMemoryIdentity({
      userId: account.userId,
      memoryKey: account.memoryKey,
      masterPassphrase: config.agent.memoryPassphrase ?? "",
      folder: `agent-${account.agentId}`,
    });
    ...
    const recalled = await memory.recall({
      ...
      query: temporalRecallQuery(text),
    });
    const recalledMemory = recalled.kind === "recalled" ? recalled.records : [];
    const approved = /^\/approve\s+/i.test(text);
    const command = approved ? text.replace(/^\/approve\s+/i, "") : text;
    const preview = await pipeline.handle({
      text: command,
    ...
    if (!config.keeperhub.apiKey) {
      await telegram.send({
        chatId,
        text: "Live KeeperHub execution is not configured.",
      });
      return NextResponse.json({ ok: true });
    }
    ...
    const result = await executeApprovedPreview({
      agentId: account.agentId,
      preview,
      recalledMemory,
      ...
      execution: createExecutionService({
        repository: createDrizzleExecutionRepository(database.db),
        keeperHub,
      }),
    });
    ...
    if (result.kind === "executed") {
      await telegram.send({
        chatId,
        text: `Auctor execution ${result.audit.status}.\nAudit: ${result.audit.id}\nTransaction: ${result.audit.transactionHash ?? "pending receipt"}`,
      });
    } else
      await telegram.send({
        chatId,
        text: `Execution stopped safely: ${"error" in result ? result.error.message : result.reason}`,
      });
    return NextResponse.json({ ok: true });
  } catch {
    await telegram.send({
      chatId,
      text: "Auctor could not process that message. No transaction was submitted.",
    });
    return NextResponse.json({ ok: true });
  } finally {
