...
export async function POST(request:Request){const config=loadConfig(runtimeEnvironment());const token=(await cookies()).get("auctor_session")?.value;if(!token)return NextResponse.json({error:"unauthorized"},{status:401});if(!config.database.url||!config.agent.memoryPassphrase)return NextResponse.json({error:"agent_unavailable"},{status:503});const database=createDatabase(config.database.url);try{const session=await resolveSession(database.db,hashToken(token));if(!session)return NextResponse.json({error:"unauthorized"},{status:401});if(!session.khOrgId||!session.khWalletAddress)return NextResponse.json({error:"wallet_not_provisioned"},{status:503});let body:unknown;try{body=await request.json()}catch{return NextResponse.json({error:"invalid_json"},{status:400})}const pipeline=createRuntimeChatPipeline({...config,keeperhub:{...config.keeperhub,walletAddress:session.khWalletAddress}});const identity=deriveMemoryIdentity({userId:session.userId,memoryKey:session.memoryKey,masterPassphrase:config.agent.memoryPassphrase,folder:`agent-${session.agentId}`});const memory=createMemoryClient({baseUrl:config.memory.url});const chatSession=createChatSession({pipeline,memory});const conversation=createConversationService({repository:createDrizzleConversationRepository(database.db),pipeline});const language=createNaturalLanguageRouter({client:createLlmClient(config.llm)});const policy=await createDrizzleAgentPolicyRepository(database.db).load(session.agentId,new Date().toISOString().slice(0,10));const response=await handleChatRequest(body,{pipeline,session:chatSession,identity,conversation,agentId:session.agentId,language,agent:{name:session.agentName,autonomyMode:session.autonomyMode,dailyCapUsd:policy?String(Number(policy.rules.maxDailyUsdMicros)/1_000_000):"0"}});return NextResponse.json(response.body,{status:response.status})}catch(error){return NextResponse.json({error:error instanceof Error?error.message:"chat_unavailable"},{status:503})}finally{await database.close()}}
